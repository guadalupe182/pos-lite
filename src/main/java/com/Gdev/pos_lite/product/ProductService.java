    ...
    public Product adjustByBarcode(ProductAdjustDto req) {
        ...
        if (p == null) {
            if (req.name() == null || req.name().isBlank() || req.price() == null) {
                throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Producto no existe. Para alta rápida envía name, price y categoryId o categoryName");
            }
            ...
            if (req.categoryId() != null) {
                cat = categoryRepo.findById(req.categoryId()).orElseThrow(() -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "categoryId inválido"));
                ...
                if (cat == null) {
                    ...
                    try {
                        ...
                        cat = categoryRepo.findByNameIgnoreCase(cn).orElseThrow(() -> new ResponseStatusException(HttpStatus.CONFLICT, "Conflicto creando categoría"));
                    }
                }
            } else {
            ...
            p = Product.builder().name(req.name().trim()).barcode(barcode).category(cat).price(// BigDecimal ya en DTO
            req.price()).stock(0).minStock(minStock).build();
            // Guardar producto con manejo de choque por barcode único
            try {
                p = productRepo.save(p);
            } catch (DataIntegrityViolationException ex) {
                // Otro request insertó el mismo barcode → re-consulta con lock
                p = productRepo.findWithLockByBarcode(barcode).orElseThrow(() -> new ResponseStatusException(HttpStatus.CONFLICT, "Conflicto de barcode"));
            }
        }
        ...
        final int newStock;
        try {
            newStock = (op == Op.IN) ? // lanza ArithmeticException si overflow int
            Math.addExact(current, qty) : Math.subtractExact(current, qty);
