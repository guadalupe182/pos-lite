...
public interface SaleRepository extends JpaRepository<Sale, Long> {
    ...
    @Query("SELECT COALESCE(SUM(s.total), 0.0) FROM Sale s WHERE s.saleDate BETWEEN :start AND :end AND s.paymentMethod = :paymentMethod")
        Double sumTotalByPaymentMethod(@Param("paymentMethod") String paymentMethod,
                                       @Param("start") Instant start,
                                       @Param("end") Instant end);
    //Agrupa por método de pago y devuelve pares (método, total)
    @Query("SELECT s.paymentMethod, COALESCE(SUM(s.total), 0.0) FROM Sale s " +
                "WHERE s.saleDate BETWEEN :start AND :end " +
                "GROUP BY s.paymentMethod")
        List<Object[]> sumTotalGroupedByPaymentMethod(@Param("start") Instant start,
                                                      @Param("end") Instant end);
