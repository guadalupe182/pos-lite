...
public class AuthService {
    ...
    public AuthService(UserRepository userRepository, PasswordEncoder passwordEncoder, JwtService jwtService, @Value("${admin.invite.code:}") String adminInviteCode) {
    ...
    public String loginAndIssueToken(LoginRequest req) {
        String email = normalizeEmail(req.getEmail());
        String password = req.getPassword();
        User u = userRepository.findByEmailIgnoreCase(email).orElseThrow(() -> new IllegalArgumentException("Credenciales inválidas"));
        if (!u.isEnabled())
        ...
        Map<String, Object> claims = Map.of("roles", u.getRoles().stream().map(Role::name).collect(Collectors.toList()));
